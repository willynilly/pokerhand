...
Poker.prototype = {
	...
	isRoyalFlush: function(hand) {
		...
		var royal_ranks = this.deck.getRanksDescending().slice(0,this.hand_size);
		var cards = hand.getCards();
		var royal_count = 0;
		var suit = cards[0].suit;
		for (var c in cards) {
			if (cards[c].suit != suit) {return false;}
			if (royal_ranks.indexOf(cards[c].rank) != -1 || this.isWildCard(cards[c])) {
				royal_count++;
			}
		}
		return royal_count == this.hand_size;
	},
	...
	isHighCard: function(hand) {
		if (!this.isValidHand(hand)) {return false;}
		var rank_counts = hand.getRankCounts();
		var suit_counts = hand.getSuitCounts();
		if (Object.keys(rank_counts).length == 5 && Object.keys(suit_counts).length >= 2) {
			return true;
		}
		return false;
	},
